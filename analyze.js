...
/**
 * Scoring khusus untuk token early (bonding curve)
 */
function analyze(p) {
  ...
  if (t5m >= 3) {
    ...
    else if (bp5m < 0.35) { score -= 12; neg.push('Sell pressure 5m'); }
  }
  if (t1h >= 6) {
    if (bp1h >= 0.65) { score += 12; pos.push('Buy% 1h kuat'); }
    else if (bp1h < 0.40) { score -= 11; neg.push('Sell dominance 1h'); }
  }
  if (t24 >= 15) {
    if (bp24 >= 0.62) { score += 8; pos.push('Buy% 24h bagus'); }
    else if (bp24 < 0.42) { score -= 9; neg.push('Sell dominance 24h'); }
  }
  ...
  // ===== AGE =====
  if (age != null) {
    if (age < 0.25) { score += 16; pos.push('Sangat fresh (<15m)'); }
    else if (age < 0.5) { score += 11; pos.push('Fresh (<30m)'); }
    else if (age < 1.5) { score += 6; pos.push('Early'); }
    else if (age > 6) { score -= 8; neg.push('Sudah agak tua'); }
  }
  ...
  const hard = neg.some(n =>
    ...
    n.includes('Sell pressure') ||
    n.includes('Sell dominance')
  );
  ...
  return {
    score, pos, neg, verdict,
    bp24, bp1h, bp5m,
    age, liq, vol24, mcap, chg24
  };
}

module.exports = { analyze, ageH };
